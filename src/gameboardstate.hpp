...
public:
	...
	explicit GameboardState(std::array<std::array<square::type,3>,3> values) : m_board{0} {
	...
	explicit GameboardState(std::initializer_list<square::type> il) : m_board{0} {
		unsigned int i{0};
	...
	explicit GameboardState(std::initializer_list<std::initializer_list<square::type>> il) : m_board{0} {
		unsigned int i{0};
	...
	explicit GameboardState(const square::type values[3][3]) : m_board{0} {
		for (uint_fast8_t i{3} ; i-- ;) {
			for (uint_fast8_t j{3} ; j-- ;) {
				set(i, j, values[i][j]);
			}
