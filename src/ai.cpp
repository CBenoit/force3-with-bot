...
heuristic::return_t AI::negamax(GameState game_state, bool is_opponent, size_t remaining_depth) const {
	if (remaining_depth && !game_state.is_there_a_winner()) {
		...
		return is_opponent
				? -(m_heuristic(game_state, game_state.get_previous_player()) + remaining_depth)
				: (m_heuristic(game_state, game_state.get_current_player()) + remaining_depth);
		/* by adding the remaining depth to the score value, winning states that are the
		   nearest of the root gets a better score (doesn't matter when it is a not winning
		   state: remaining depth is equal to 0 in that case. */
	}
}
